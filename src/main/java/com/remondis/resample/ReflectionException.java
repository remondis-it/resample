...
public class ReflectionException extends RuntimeException {
	...
	static ReflectionException zeroInteractions(String configurationMethod) {
		return new ReflectionException(String.format(
				"The field selector for method '%s' tracked zero interactions with properties.", configurationMethod));
	}
	...
	static ReflectionException notAGetter(Method method) {
		return new ReflectionException(
				String.format("The method '%s' in type %s is not a valid Java Bean property get-method.",
	...
	static ReflectionException noDefaultConstructor(Class<?> type) {
		return new ReflectionException(
				String.format("The type %s does not have a public no-args constructor and cannot be used for mapping.",
						type.getName()));
	}
	...
	static ReflectionException unsupportedCollection(Collection<?> collection) {
		return new ReflectionException(
				String.format("The collection '%s' is currently not supported. Only java.util.Set and java.util.List"
						+ " are supported collections.", collection.getClass().getName()));
	}
